...
Last, go to **Developers → Webhooks**, click "Add endpoint," and enter `https://YOUR-RENDER-URL/api/stripe/webhook` (using your actual Render address) as the endpoint URL. Select **three** events to listen for: `checkout.session.completed` (tells your app when a client has actually paid, flipping their status to "paid" automatically), `customer.subscription.deleted` (tells your app the moment a subscription actually ends, flipping their status to "cancelled" automatically — this is also your cue to pause or remove their agent in Retell once that's connected, since cancelling billing doesn't by itself turn off a live phone agent), and `invoice.paid` (tells your app a client's billing period has renewed, which resets their monthly call/chat minute usage back to zero — this is what makes the automatic overage billing described in Step 11 below start fresh each month instead of carrying minutes over forever). Stripe gives you a **signing secret** starting with `whsec_` — add that to Render as `STRIPE_WEBHOOK_SECRET`.
...
## Step 11 — Automatic minute tracking & overage billing (no extra setup needed)

This one needs no new environment variables and no dashboard clicking -- it starts working automatically as soon as Step 5 (Retell) and Step 6 (Stripe, including the `invoice.paid` event above) are both done, and it's here mainly so you know what to expect rather than as a task to do.

Once both are set up, every client's real phone-call time (from Retell) and website-widget chat activity (a fixed estimate per message) count toward the "250" or "750 call & chat minutes a month" their plan promises. Their dashboard card shows a live usage bar. If a client goes over, extra minutes are billed automatically at £0.35/minute (Starter) or £0.30/minute (Growth) -- the same range already stated on the pricing page -- added as an extra line on their next regular Stripe invoice, no separate charge or receipt. Before that happens, the client gets an automatic email at 80% of their allowance, and another one the moment they go over, so a bigger-than-usual bill is never a surprise. None of this ever slows down or blocks the AI itself for the customer on the other end of the call or chat.
