...
Next, go to **Product catalog** and create two products — one named "Starter" priced at $149/month (recurring), one named "Growth" priced at $299/month (recurring). Each one gives you a Price ID that looks like `price_1AbCdEfG...` — copy those and add them to Render as `STRIPE_PRICE_ID_STARTER` and `STRIPE_PRICE_ID_GROWTH`.

Last, go to **Developers → Webhooks**, click "Add endpoint," and enter `https://YOUR-RENDER-URL/api/stripe/webhook` (using your actual Render address) as the endpoint URL. Select the `checkout.session.completed` event to listen for. Stripe gives you a **signing secret** starting with `whsec_` — add that to Render as `STRIPE_WEBHOOK_SECRET`. This webhook is what tells your app automatically when a client has actually paid, so their status flips to "paid" in your dashboard without you having to check anything manually.
